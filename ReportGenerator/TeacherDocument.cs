...
public class TeacherDocument: IDocument
{
    private SisContext context { get; }
    public TeacherDocument(SisContext context)
    {
    ...
    private void ComposeData(IContainer container)
    {
        container.Table(table =>
        {
            ...
            foreach (var user in context.Users.Include(u => u.Teacher).Where(u => u.Teacher!.Status == 1))
            {
                ...
                table.Cell().Element(CellStyle).Text(user.Teacher?.Department);
